...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    api.get('/schedules')
      ...
      .catch(err => {
        ...
        toast.error('تعذّر تحميل الجدول')
      })
  ...
  // استخدام getDay() الرقمي — موثوق ولا يعتمد على locale strings
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        ...
        <CardDescription>لوحة تحكم الدكتور</CardDescription>
      </div>

      <div>
        <h2 className="mb-4 text-xs font-bold tracking-widest text-slate-400 uppercase">نظرة عامة</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="transition-all duration-200 hover:shadow-md">
            <CardContent className="flex items-center gap-5 px-6 py-5">
              ...
              <div className="min-w-0">
                <p className="text-xs font-medium text-slate-400 truncate">محاضرات اليوم</p>
          ...
          <Card className="transition-all duration-200 hover:shadow-md">
            <CardContent className="flex items-center gap-5 px-6 py-5">
              ...
              <div className="min-w-0">
                <p className="text-xs font-medium text-slate-400 truncate">إجمالي المحاضرات</p>
          ...
          <Card className="transition-all duration-200 hover:shadow-md">
            <CardContent className="flex items-center gap-5 px-6 py-5">
              ...
              <div className="min-w-0">
                <p className="text-xs font-medium text-slate-400 truncate">أيام المحاضرات</p>
      ...
      <Card>
        ...
        <CardContent>
          {todaySchedules.length === 0 ? (
            <p className="text-center text-slate-400 py-8">لا توجد محاضرات اليوم</p>
          ) : (
              ...
              {todaySchedules.map(sch => (
                <div key={sch.id} className="flex flex-col gap-3 rounded-lg bg-slate-50 p-4 transition-colors hover:bg-blue-50 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="sm:text-end">
                    <p className="text-sm font-medium text-slate-700">{sch.hallId?.name || 'مدرج'}</p>
